...
def scan_callback(scan_data):
    ...
    min_distance = get_minimum_front_range(scan_data, 5, 5)
    ...
    # print("min_between_indices 0 y 5: ",
    #       min_between_indices(scan_data.ranges, 0, 5))

    # print("min front", get_minimum_front_range(scan_data, 5, 5))
...
def min_between_indices(ranges, i, j):
    ranges = [x for x in ranges if not math.isnan(x)]
    if (len(ranges) != 0):
        slice_of_array = ranges[i: j+1]
...
def get_minimum_front_range(scan_data, left_angle, right_angle):
    global _scanMsg
    right_min_data = min_between_indices(_scanMsg.ranges, 0, right_angle)
    left_min_data = min_between_indices(_scanMsg.ranges, len(
        _scanMsg.ranges)-left_angle, len(_scanMsg.ranges))
    # print("front_range_ ", min([left_min_data, right_min_data]))
    return min([left_min_data, right_min_data])
...
def move(avoid_obstacle):
    ...
    global min_distance
    global _scanMsg
    get_minimum_front_range(_scanMsg, 5, 5)
    velocity_publisher = rospy.Publisher(
        "/cmd_vel", Twist, queue_size=10)
    ...
    while(True):
        if (avoid_obstacle):
            ...
            cmd_vel.linear.x = -0.5*(0.7-min_distance)
            cmd_vel.angular.z = 0.0
            print("lineal vel: ", cmd_vel.linear.x)
